...
// JavaScript for the key reset process and loading overlay
...
resetBtn.addEventListener('click', function() {
  // Show the loading overlay and start the progress bar
  loadingOverlay.style.display = 'flex';
  
  let progress = 0;
  loadingBar.style.width = '0%';  // Start the loading bar at 0%

  // Simulate loading progress
  let interval = setInterval(() => {
    progress += 2;  // Increase the progress
    loadingBar.style.width = `${progress}%`;  // Corrected template literal

    if (progress >= 100) {
      clearInterval(interval);  // Stop the progress once it reaches 100%
      setTimeout(() => {
        // Show the success overlay after a small delay
        loadingOverlay.style.display = 'none';
        successOverlay.style.display = 'flex';
      }, 300);
    }
  }, 100);  // Update the bar every 100ms (adjust to your liking)
});

// Close success overlay when the close button is clicked
